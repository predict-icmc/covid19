...
dados1$tempo<- as.numeric(as.Date(dados$date) - min(as.Date(dados$date)))

latlong_cidade<-read.csv('/home/cibele/Google Drive/PREDICT COVID-19/covid19/dados/latitude-longitude-cidades.csv', sep=';', header=TRUE)
latlong_cidade$city <-latlong_cidade$municipio
latlong_cidade$state <-latlong_cidade$uf

latlong_estado<-read.csv('/home/cibele/Google Drive/PREDICT COVID-19/covid19/dados/latitude-longitude-estados.csv', sep=';', header=TRUE)
